...
Residual limitation, left alone deliberately: at `maxDepth: 0` a relative deep import is still rewritten to `'.'` and siblings still collapse. At that setting the rule's own `error` message also says `Import from "."`, so message and fixer agree; the real issue there is that `maxDepth: 0` is an unsatisfiable policy for relative specifiers, which is a separate question from this fix.
