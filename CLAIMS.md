...
| "58 findings per 1k files vs 985" | ilb-real-source | [benchmark-2026-08-14.json](benchmarks/results/published/benchmark-2026-08-14.json) — 20 OSS projects, 23,682 files, 2.37M SLOC, every repo pinned to a commit. A RAW count — total findings divided by file count. It does not come from the hand-labelled precision sample, which supports only the true/false-positive estimates below | 2026-08-14 |
| "Measured precision ~67% on real open-source code" (was 47% before 2026-08-14) | ilb-real-source + hand labelling | 22-finding stratified sample, hand-labelled 12 TP / 6 FP / 4 undecidable. **A sample, not a census** — drawn from ~13 of 20 repos. Improved from 47% by removing name-matching in no-xpath-injection, no-improper-sanitization and no-http-urls; each measured before and after with the committed runner | 2026-08-14 |
...
| "Every plugin's API surface is enumerated at its installed version, and coverage is published as an UPPER BOUND — never as a pass mark. 20 plugins have an enumerable surface; 10 analyse plain JS or the web platform and are reported as such rather than given a percentage. The 60% floor applies only where the denominator has been curated to the consumer-callable API" | [`.agent/api-surface-manifest.json`](.agent/api-surface-manifest.json) + [`benchmark-results/api-surface-coverage.md`](benchmark-results/api-surface-coverage.md) | [`scripts/measure-api-surface.mts`](scripts/measure-api-surface.mts) via [`scripts/audit-api-surface.ts`](scripts/audit-api-surface.ts) + [`.github/workflows/api-surface.yml`](.github/workflows/api-surface.yml) (PR gate; a measurement or scope violation fails it, and recorded below-floor debt warns — `--strict` escalates those and is deliberately not what CI runs) | 2026-09-09 |
