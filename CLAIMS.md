...
| "Every plugin's API surface is enumerated at its installed version, and coverage is published as an UPPER BOUND — never as a pass mark. 20 plugins have an enumerable surface; 10 analyse plain JS or the web platform and are reported as such rather than given a percentage. The 60% floor applies only where the denominator has been curated to the consumer-callable API" | [`.agent/api-surface-manifest.json`](.agent/api-surface-manifest.json) + [`benchmark-results/api-surface-coverage.md`](benchmark-results/api-surface-coverage.md) | [`scripts/measure-api-surface.mts`](scripts/measure-api-surface.mts) via [`scripts/audit-api-surface.ts`](scripts/audit-api-surface.ts) + [`.github/workflows/api-surface.yml`](.github/workflows/api-surface.yml) (PR gate, strict) | 2026-09-09 |
