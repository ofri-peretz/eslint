...
description: "Detects instances of child_process & non-literal exec() calls that may allow command injection. This rule is part of eslint-plugin-secure-coding and provides LL"
---
...
```
⚠️ CWE-78 OWASP:A03-Injection CVSS:9.8 | Command injection detected | CRITICAL [SOC2,PCI-DSS]
...
| ---------------- | --------------------------- | ------------------------------------ |
| **CWE**          | Vulnerability identifier    | `CWE-78`                             |
| **OWASP**        | OWASP Top 10 category       | `OWASP:A03-Injection`                |
| **CVSS**         | Severity score              | `CVSS:9.8`                           |
| **Description**  | Issue summary               | `Command injection detected`         |
| **Severity**     | Risk level                  | `CRITICAL`                           |
| **Compliance**   | Affected frameworks         | `[SOC2,PCI-DSS]`                     |
| **Fix**          | Recommended remediation     | `Use execFile/spawn with {shell: false}` |
| **Documentation**| Link to detailed guidance   | OWASP attack guide URL               |
...
<Callout type="info">
  **📚 Learn more about static analysis limitations**: These patterns escape detection because ESLint analyzes code without executing it. See [Static Analysis Limitations](/docs/concepts/static-analysis) to understand what all security rules can and cannot detect—and how to layer your defenses.
</Callout>
