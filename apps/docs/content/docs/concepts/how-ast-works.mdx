...
flowchart LR
    ...
    C -->|Yes| D["🚨 Report: Possible SQL Injection"]
    B -->|No| E[✅ Pass]
    C -->|No| E
```
...
<div className="grid sm:grid-cols-2 gap-4 my-6">
  ...
  <div className="p-4 rounded-lg border border-fd-border bg-violet-500/5">
    <h3 className="text-lg font-semibold text-violet-400 mb-2">🔗 CSS-like Selectors</h3>
  ...
  <div className="p-4 rounded-lg border border-fd-border bg-violet-500/5">
    <h3 className="text-lg font-semibold text-violet-400 mb-2">📍 Context API</h3>
...
flowchart LR
    subgraph "Static Analysis"
        A["📁 100 files"] --> B["🔍 Parse & Check"]
        B --> C["~150ms total"]
    end
    subgraph "Runtime Testing"
        D["📁 100 files"] --> E["🚀 Start Server"]
        E --> F["Run Tests"]
        F --> G["~30s+ total"]
    end
```
...
| **Parsing** | Source → AST happens once per file |
