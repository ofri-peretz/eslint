...
[`no-innerhtml`](./no-innerhtml) / [`no-eval`](./no-eval), which report it
...
- [`browser-security/require-postmessage-origin-check`](./require-postmessage-origin-check) - Require origin validation
- [`browser-security/no-postmessage-wildcard-origin`](./no-postmessage-wildcard-origin) - Prevent wildcard targetOrigin
- [`browser-security/no-innerhtml`](./no-innerhtml) - General innerHTML prevention
