...
- [`no-innerhtml`](./no-innerhtml) - Prevent XSS that could access localStorage
- [`require-postmessage-origin-check`](./require-postmessage-origin-check) - Cross-origin security
