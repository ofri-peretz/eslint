...
- [`no-disabled-certificate-validation`](./no-disabled-certificate-validation) - Prevent SSL bypass
- [`no-insecure-websocket`](./no-insecure-websocket) - Require WSS (secure WebSocket)
