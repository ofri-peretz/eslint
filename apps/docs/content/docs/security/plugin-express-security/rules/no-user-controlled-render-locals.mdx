...
- [`no-user-controlled-redirect`](./no-user-controlled-redirect) - Open redirect from request input
- [`no-static-root-exposure`](./no-static-root-exposure) - Application root served statically
