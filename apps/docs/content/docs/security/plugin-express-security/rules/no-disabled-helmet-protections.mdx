...
`hsts` / `strictTransportSecurity` belong to [`require-strict-transport-security`](./require-strict-transport-security), and CSP *directive contents* to [`no-unsafe-csp-directives`](./no-unsafe-csp-directives) — no finding is reported twice.
