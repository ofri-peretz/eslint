...
- [`no-verbose-error-messages`](./no-verbose-error-messages) - Prevents leaking stack traces.
- [`no-pii-in-logs`](./no-pii-in-logs) - Prevents leaking user data in logs.
- [`detect-object-injection`](./detect-object-injection) - Prevents prototype pollution in admin logic.
