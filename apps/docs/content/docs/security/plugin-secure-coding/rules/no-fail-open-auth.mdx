...
- [`no-missing-authentication`](./no-missing-authentication) - Endpoints with no authentication at all
- [`require-backend-authorization`](./require-backend-authorization) - Client-side-only authorization checks
- [`no-unhandled-stream-error`](./no-unhandled-stream-error) - Uncaught exceptions from unhandled stream errors