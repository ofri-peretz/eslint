...
- [no-operator-injection](./no-operator-injection) - Detects $ne, $gt, $or injection specifically
- [no-unsafe-where](./no-unsafe-where) - Prevents $where operator RCE
- [require-schema-validation](./require-schema-validation) - Requires Mongoose validators
