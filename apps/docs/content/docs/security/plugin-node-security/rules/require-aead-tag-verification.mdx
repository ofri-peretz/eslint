...
- the algorithm is **computed** — that is [`no-dynamic-algorithm-selection`](./no-dynamic-algorithm-selection)'s finding, not this one's.
...
- [`no-ecb-mode`](./no-ecb-mode) — Detect ECB mode
- [`no-static-iv`](./no-static-iv) — Detect hardcoded initialization vectors
- [`no-weak-cipher-algorithm`](./no-weak-cipher-algorithm) — Detect weak encryption algorithms
