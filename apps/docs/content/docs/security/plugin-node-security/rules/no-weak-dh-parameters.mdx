...
- [`no-weak-cipher-algorithm`](./no-weak-cipher-algorithm) — weak symmetric ciphers
- [`no-weak-hash-algorithm`](./no-weak-hash-algorithm) — weak digests
- [`no-insecure-rsa-padding`](./no-insecure-rsa-padding) — RSA padding modes
