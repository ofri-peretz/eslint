...
- [`detect-non-literal-fs-filename`](./detect-non-literal-fs-filename) - Path traversal in fs operations
- [`no-toctou-vulnerability`](./no-toctou-vulnerability) - Race condition vulnerabilities