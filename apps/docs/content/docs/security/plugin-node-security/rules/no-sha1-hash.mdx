...
- [`no-weak-hash-algorithm`](./no-weak-hash-algorithm) - Detects weak hash algorithms in Node.js crypto
- [`no-weak-cipher-algorithm`](./no-weak-cipher-algorithm) - Prevents weak encryption algorithms
- [`prefer-native-crypto`](./prefer-native-crypto) - Prefer Node.js native crypto over third-party packages
