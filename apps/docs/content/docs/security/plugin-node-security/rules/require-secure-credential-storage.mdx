...
`fs.writeFile()` is **not** this rule; disk writes belong to [`require-storage-encryption`](./require-storage-encryption).
...
- [`no-hardcoded-credentials`](./no-hardcoded-credentials) - Prevent hardcoded passwords
- [`require-storage-encryption`](./require-storage-encryption) - General storage encryption
...
[`require-storage-encryption`](./require-storage-encryption) owns those, and
...
[`require-storage-encryption`](./require-storage-encryption).
