...
- [`detect-n-plus-one-queries`](./detect-n-plus-one-queries) - Detects N+1 query patterns
- [`no-blocking-operations`](./no-blocking-operations) - Detects blocking operations
