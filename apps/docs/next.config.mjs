...
function cspReportOnlyHeaders() {
  ...
  // characters, so the shape check is both a correctness guard and the
  // injection guard. A malformed value drops the header rather than emitting
  // a corrupted one.
...
const config = {
  ...
  rewrites: async () => [
    ...
    // violation report 404'd. Silently: a report-uri that fails is invisible
    // from the page, and the empty event stream reads exactly like a clean
