...
This document is the sibling of [`UX_PHILOSOPHY.md`](?path=/docs/philosophy-foundations-ux--docs) and
...
([`UX_PHILOSOPHY.md` #1](?path=/docs/philosophy-foundations-ux--docs#1-adoption-is-friction-subtraction)).
A tie is a loss.
...
motion budget defined in [`MOTION_PHILOSOPHY.md`](?path=/docs/philosophy-foundations-motion--docs).
...
([`UX_PHILOSOPHY.md` #9](?path=/docs/philosophy-foundations-ux--docs#9-the-docs-site-is-the-showcase));
...
- Builds on [`UX_PHILOSOPHY.md` #11 (Build with the ecosystem)](?path=/docs/philosophy-foundations-ux--docs#11-build-with-the-ecosystem-not-against-it):
...
- **Hover** changes fill or border, never geometry. A CTA that grows on
  hover causes layout shift ([`MOTION_PHILOSOPHY.md`](?path=/docs/philosophy-foundations-motion--docs)
  CLS=0).
...
- **Loading** replaces the trailing icon with a spinner and locks the label
  ...
  [`LOADING_PHILOSOPHY.md`](?path=/docs/philosophy-interaction-states-loading--docs) for the spinner pattern.)
