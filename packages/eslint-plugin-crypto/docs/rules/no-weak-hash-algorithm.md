...
| **Auto-Fix**   | ✅ Yes (via suggestions to SHA-256)     |
| **Category**   | Security |
...
```text
🔒 CWE-327 OWASP:A02 | Broken Hash Algorithm detected | CRITICAL [BrokenCrypto]
...
flowchart TD
    ...
    B -->|Yes| C[🚨 Collision Risk]
    B -->|No| D[✅ Recommended Algorithm]
    C --> E[💡 Suggest SHA-256 / SHA-512]
```
...
| 🚀 **Integrity Bypass** | Malicious code accepted as valid    | Standardize on SHA-256 for all checksums and fingerprinting |
| 🔒 **Compliance**       | Failure to meet NIST/FIPS standards | Decommission all MD5/SHA1 usage for non-legacy use cases    |
...
crypto.createHash(algo); // ❌ NOT DETECTED
```
