...
| **Auto-Fix**   | ✅ Yes (via suggestion to use IV variant)     |
| **Category**   | Security |
...
```text
🔒 CWE-327 OWASP:A02 | Deprecated crypto method detected | HIGH [BrokenCrypto]
...
flowchart TD
    ...
    B -->|Yes| C[🚨 Deprecated / Insecure Risk]
    B -->|No - is iv variant| D[✅ Recommended API]
    C --> E[💡 Suggest createCipheriv / createDecipheriv]
```
...
| 🚀 **Weak Salt/Key** | Rapid password brute-forcing        | Derive keys using `PBKDF2` or `scrypt` before encrypting     |
| 🔒 **Compliance**    | Violates modern security baselines  | Decommission all legacy Node.js crypto calls from production |
...
const c = crypto[method](...); // ❌ NOT DETECTED
```
