 ...
 * `require('eslint-plugin-jwt-security/oxlint')` view, it sits at the top of
...
describe('eslint-plugin-jwt-security/oxlint sub-export', () => {
  ...
  it('re-exports the plugin object (meta + rules at top level)', async () => {
    ...
    expect(oxlintPlugin.meta?.name).toBe('eslint-plugin-jwt-security');
