...
exports.formatLLMMessage = formatLLMMessage;
exports.formatLLMMessageTemplate = formatLLMMessageTemplate;
exports.getSecurityBenchmarks = getSecurityBenchmarks;
...
exports.clearMessageTemplates = clearMessageTemplates;
 ...
 * ```
 */
function formatLLMMessage(options) {
 ...
 */
function formatLLMMessageTemplate(options) {
 ...
 */
function getSecurityBenchmarks(cwe) {
    const cweData = constants_1.CWE_MAPPING[cwe];
    if (!cweData)
        return undefined;
    const owaspDetails = constants_1.OWASP_DETAILS[cweData.owasp];
    return {
        owasp: cweData.owasp,
        ...
        name: cweData.name,
        compliance: constants_1.CWE_COMPLIANCE_MAPPING[cwe] ?? [],
    };
}
// ============================================================================
 ...
 * ```
 */
function registerMessageTemplate(name, config) {
 ...
 */
function clearMessageTemplates() {
 ...
 * ```
 */
function formatWithTemplate(options) {
    ...
    const owasp = enriched.owasp;
    const cvss = enriched.cvss ??
        (0, constants_1.severityToCVSS)(enriched.severity);
    const compliance = enriched.compliance ?? [];
    ...
    const complianceDisplay = allCompliance.length > 0 ? `[${allCompliance.slice(0, 4).join(',')}]` : '';
    ...
    const line1Template = templateConfig.line1Format ??
 ...
 */
function toSARIF(options) {
    const enriched = enrichFromCWE(options);
    return {
        ruleId: options.ruleId ||
            options.cwe ||
            options.issueName.replace(/\s+/g, '-').toLowerCase(),
        level: severityToSARIFLevel(enriched.severity),
        message: { text: enriched.description },
        properties: {
            'security-severity': enriched.cvss?.toString() ??
                (0, constants_1.severityToCVSS)(enriched.severity).toString(),
            cwe: enriched.cwe,
