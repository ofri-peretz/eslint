...
exports.detectResolvers = detectResolvers;
exports.generateRecommendedConfig = generateRecommendedConfig;
exports.migrateFromEslintImport = migrateFromEslintImport;
exports.validateResolverConfig = validateResolverConfig;
const fs_1 = require("../node/fs");
/**
 * Detect available resolvers based on project structure
 */
function detectResolvers(workspaceRoot) {
    const results = [];
    // TypeScript resolver
    const tsconfigPath = (0, fs_1.findFileUpward)('tsconfig.json', workspaceRoot);
    results.push({
        name: 'typescript',
    ...
    // Webpack resolver
    const webpackConfig = (0, fs_1.findFileUpward)('webpack.config.js', workspaceRoot) ||
        (0, fs_1.findFileUpward)('webpack.config.ts', workspaceRoot);
    results.push({
        name: 'webpack',
    ...
    // Vite resolver
    const viteConfig = (0, fs_1.findFileUpward)('vite.config.js', workspaceRoot) ||
        (0, fs_1.findFileUpward)('vite.config.ts', workspaceRoot) ||
        (0, fs_1.findFileUpward)('vite.config.mjs', workspaceRoot);
    results.push({
        name: 'vite',
        detected: !!viteConfig,
    ...
    // Rollup resolver
    const rollupConfig = (0, fs_1.findFileUpward)('rollup.config.js', workspaceRoot) ||
        (0, fs_1.findFileUpward)('rollup.config.ts', workspaceRoot);
    results.push({
        name: 'rollup',
    ...
    const packageJson = (0, fs_1.findFileUpward)('package.json', workspaceRoot);
    let hasReact = false;
    if (packageJson) {
        try {
 ...
 */
function generateRecommendedConfig(workspaceRoot) {
 ...
 */
function migrateFromEslintImport(oldConfig) {
    const migrated = {};
    const warnings = [];
    const suggestions = [];
    // Migrate settings
    ...
    if (oldRules && typeof oldRules === 'object' && oldRules !== null) {
        const newRules = {};
        Object.entries(oldRules).forEach(([ruleName, ruleConfig]) => {
 ...
 */
function validateResolverConfig(settings) {
    const errors = [];
    const warnings = [];
