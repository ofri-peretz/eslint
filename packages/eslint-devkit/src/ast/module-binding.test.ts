    ...
    create(context) {
      return {
        CallExpression(node: TSESTree.CallExpression) {
          ...
          if (
            isModuleBinding(node.callee, scope, module, exportPath, options)
          ) {
...
describe('resolveModuleBinding', () => {
  ruleTester.run(
    'child_process.exec through every binding shape',
    makeProbe('child_process', ['exec']),
    {
      valid: [
      ...
      invalid: [
        ...
        {
          code: `const cp = require('node:child_process'); cp.exec(c);`,
          errors: 1,
        },
        { code: `import cp from 'node:child_process'; cp.exec(c);`, errors: 1 },
        {
          code: `import * as cp from 'node:child_process'; cp.exec(c);`,
          errors: 1,
        },
        ...
        {
          code: `const { exec } = require('node:child_process'); exec(c);`,
          errors: 1,
        },
        {
          code: `const { exec: run } = require('child_process'); run(c);`,
          errors: 1,
        },
        { code: `import { exec } from 'child_process'; exec(c);`, errors: 1 },
        {
          code: `import { exec as run } from 'node:child_process'; run(c);`,
          errors: 1,
        },
        ...
        {
          code: `const run = require('child_process').exec; run(c);`,
          errors: 2,
        },
        // A STRING SUBSCRIPT names the same export its dotted spelling names, and
        // it is what bundlers and minifiers emit. Nothing else in the workspace
        // pins this: reverting the member arm to `property.name` left all 1836
        // devkit tests green AND the computed-key probe at zero, because every
        // rule that reaches this resolver now also has its own gate.
        {
          code: `const cp = require('child_process'); cp['exec'](c);`,
          errors: 1,
        },
        { code: `require('node:child_process')['exec'](c);`, errors: 1 },
      ],
  ...
  ruleTester.run(
    'a key chosen at RUNTIME names no export',
    makeProbe('child_process', ['exec']),
    {
      // The two shapes the resolver must keep abstaining on. `propertyName` is
      // narrower than "computed is fine": a runtime key could be anything, and a
      // `#private` field can never be a module export at all.
      valid: [
        { code: `const cp = require('child_process'); cp[method](c);` },
        {
          code: `const cp = require('child_process'); const k = 'exec'; cp[k](c);`,
        },
      ],
      invalid: [],
    },
  ...
  ruleTester.run(
    'fs.promises.readFile through nested namespaces',
    makeProbe('fs', ['promises', 'readFile']),
    {
      valid: [{ code: `const fs = require('fs'); fs.readFile(f);` }],
      invalid: [
        { code: `const p = require('fs').promises; p.readFile(f);`, errors: 1 },
        {
          code: `const p = require('node:fs').promises; p.readFile(f);`,
          errors: 1,
        },
        {
          code: `const fs = require('fs'); const { readFile } = fs.promises; readFile(f);`,
          errors: 1,
        },
        {
          code: `const fs = require('fs'); const { readFile: alias } = fs.promises; alias(f);`,
          errors: 1,
        },
        // Subscripted at BOTH levels of the namespace, not just the leaf.
        {
          code: `const p = require('fs')['promises']; p['readFile'](f);`,
          errors: 1,
        },
      ],
    },
  ...
  ruleTester.run(
    'module equivalents are configurable',
    makeProbe('fs', ['readFile'], {
      equivalents: { 'fs-extra': 'fs', 'graceful-fs': 'fs' },
    }),
    {
      valid: [{ code: `const x = require('unrelated-pkg'); x.readFile(f);` }],
      invalid: [
        {
          code: `const fse = require('fs-extra'); fse.readFile(f);`,
          errors: 1,
        },
        {
          code: `import { readFile } from 'fs-extra'; readFile(f);`,
          errors: 1,
        },
  ...
  ruleTester.run(
    'module root matches when no export path is given',
    makeProbe('child_process', undefined),
    {
      valid: [{ code: `const x = require('other'); x.y();` }],
      invalid: [
        {
          code: `const cp = require('node:child_process'); cp.spawn(c);`,
          errors: 1,
        },
      ],
    },
  ...
  ruleTester.run(
    ...
    makeProbe('fs', ['readFile']),
    {
      ...
      invalid: [
        ...
        {
          code: `const { 'readFile': alias } = require('fs'); alias(p);`,
          errors: 1,
        },
      ],
    },
...
describe('ESM import forms', () => {
  ruleTester.run('import shapes', makeProbe('fs', ['readFile']), {
    ...
    invalid: [
      ...
      {
        code: `import { 'readFile' as alias } from 'fs'; alias(p);`,
        errors: 1,
      },
    ],
  });
...
describe('defensive paths', () => {
  ruleTester.run('never throws, never guesses', makeProbe('fs', ['readFile']), {
    valid: [
      ...
      {
        code: `class C { #fs; m() { this.#fs.readFile(p); } }`,
        filename: 'a.ts',
      },
...
const sweepRule = createRule({
  ...
  create(context) {
    return {
      '*'(node: TSESTree.Node) {
        const result = resolveModuleBinding(
          node,
