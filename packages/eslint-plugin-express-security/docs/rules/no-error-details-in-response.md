...
> **Keywords:** stack trace exposure, error message leak, CWE-209, information disclosure, err.stack, express error handling, error middleware, verbose errors, reconnaissance
...
### A property named at runtime

**Why**: a key chosen at runtime names no property, so there is nothing to
match against the sensitive-field list. A STATIC subscript is resolved and
reported — `res['send'](err['stack'])` is detected exactly as
`res.send(err.stack)` is.
...
- [`no-exposed-debug-endpoints`](./no-exposed-debug-endpoints.md) - Prevents exposed debug/admin routes.
- [`no-host-header-in-links`](./no-host-header-in-links.md) - Prevents Host-header poisoning of mailed links.
