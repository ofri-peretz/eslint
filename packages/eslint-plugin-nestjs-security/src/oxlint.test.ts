 ...
 * `require('eslint-plugin-nestjs-security/oxlint')` view, it sits at the top
 * of `module.exports` — that's exactly the `{ meta, rules }` shape oxlint's
 * JS plugin loader expects.
 */

type Plugin = typeof mainPlugin;

describe('eslint-plugin-nestjs-security/oxlint sub-export', () => {
