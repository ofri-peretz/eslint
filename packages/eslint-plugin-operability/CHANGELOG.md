...
All notable changes to `eslint-plugin-operability` are documented here.
...
- **🧹 Refactor** — the verbose-error check no longer casts an unnameable member
...
- **💥 Breaking** — **💥 Breaking** — `require-data-minimization` no longer guesses what your PII is
...
- **✨ Feature** — **✨ Feature** — `require-code-minification` gains `minificationKeys`
