...
export const cognitiveComplexity = createRule<RuleOptions, MessageIds>({
  ...
  meta: {
    ...
    messages: {
      ...
      highCognitiveComplexity: formatLLMMessage({
        ...
        cwe: 'CWE-1104',
        description:
          '{{functionName}}: {{complexity}}/{{max}} ({{overBy}} over)',
        severity: 'HIGH',
  ...
  create(context: TSESLint.RuleContext<MessageIds, RuleOptions>) {
    const { maxComplexity = 15 }: Options = context.options[0] || {};
    ...
    function calculateCognitiveComplexity(
      ...
      function traverse(n: TSESTree.Node, currentNesting: number) {
        ...
        if (n.type === 'SwitchStatement') {
          ...
          n.cases.forEach((c: TSESTree.SwitchCase) =>
            traverse(c, currentNesting + 1),
          );
          return;
        }
    ...
    function suggestExtractions(
      ...
      if (breakdown.conditionals >= 5 && node.loc) {
        suggestions.push({
          ...
          lineRange: [node.loc.start.line, node.loc.end.line],
          estimatedComplexityReduction: Math.floor(
            breakdown.conditionals * 0.8,
          ),
        });
      }
     ...
     */
    function estimateRefactoringTime(
      complexity: number,
      breakdown: ComplexityBreakdown,
    ): string {
    ...
    function checkFunction(
      ...
      const { total: complexity, breakdown } =
