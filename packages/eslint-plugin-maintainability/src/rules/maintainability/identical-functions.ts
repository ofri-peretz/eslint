...
export interface Options {
  ...
  /** Ignore files whose header declares them machine-generated. Default: true */
...
export const identicalFunctions = createRule<RuleOptions, MessageIds>({
  ...
  create(context: TSESLint.RuleContext<MessageIds, RuleOptions>) {
    ...
    //
    // The predicate used to be `/\.(test|spec)\.[jt]sx?$/` — a BASENAME test,
    // which only recognises a suite that names its files `foo.test.ts`. Okta
    // keeps its entire suite under `test/spec/**` with plain names
    // (`test/spec/TokenManager/browser.ts`), and so the option that is on by
    // default did nothing there. On the pinned 8-repository corpus 1,961 of
    // 3,707 findings — 52.9% — were in test code, and the largest single group
    // was okta-auth-js.
    //
    // A repeated `it()` block is not a maintenance defect. The repetition IS
    // the test: each case restates its setup so it reads independently when it
    // fails. "Extract to a reusable function" is advice nobody takes, which
    // under the effective-FP standard counts against the rule exactly as a
    // wrong finding does.
    //
    // `isTestFilePath` is the devkit's shared predicate: basename for
    // `*.test.*`, exact path SEGMENT equality for the directory case. Segment
    // equality matters — `filename.includes('test')` makes the verdict depend
    // on where the repo is checked out, which has shipped here once already.
    if (ignoreTestFiles && isTestFilePath(filename)) {
      return {};
    }

    // Skip machine-generated files if configured.
    //
    // After the test-file fix above, twilio-node was 1,320 of the 1,676
    // findings left on the pinned corpus — 78.8% — and effectively all of it
    // was `src/rest/**`, which OpenAPI Generator emits. Every resource gets the
    // same `fetch`, `remove` and `page`, so the duplication is real and the
    // detection is correct. It is also unactionable: the tree is rewritten
    // wholesale on the next generator run, and the header says "Do not edit the
    // class manually".
    //
    // Decided on the file's OWN declaration — an `@generated` docblock or a
    // "this code was generated by" banner in the leading comments — not on
    // whether the path contains the word. See `isGeneratedFile`.
    if (ignoreGeneratedFiles && isGeneratedFile(sourceCode)) {
      return {};
    }
