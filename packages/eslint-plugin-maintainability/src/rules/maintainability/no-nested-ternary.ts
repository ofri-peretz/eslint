...
export const noNestedTernary = createRule<RuleOptions, MessageIds>({
  ...
  create(context: TSESLint.RuleContext<MessageIds, RuleOptions>) {
    ...
    function hasNestedTernary(node: TSESTree.ConditionalExpression): boolean {
      ...
      function containsTernary(expr: TSESTree.Node): boolean {
        ...
        switch (expr.type) {
          ...
          case 'ObjectExpression':
            return expr.properties.some(
              (prop: TSESTree.Property | TSESTree.SpreadElement) =>
                prop.type === 'Property' &&
                prop.value &&
                containsTernary(prop.value as TSESTree.Expression),
            );
          case 'CallExpression':
            return (
              expr.arguments.some(
          ...
          case 'TemplateLiteral':
            return expr.expressions.some((exp: TSESTree.Expression) =>
              containsTernary(exp),
            );
          // For tagged templates, check tag and expressions
          case 'TaggedTemplateExpression':
            return (
              containsTernary(expr.tag) ||
              expr.quasi.expressions.some((exp: TSESTree.Expression) =>
                containsTernary(exp),
              )
            );
      ...
      return (
        containsTernary(node.consequent) || containsTernary(node.alternate)
      );
    }
