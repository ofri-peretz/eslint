...
describe('no-unhandled-promise — a call is not a promise', () => {
  ruleTester.run('evidence', noUnhandledPromise, {
    valid: [
      ...
      { name: 'a wrapped promise whose chain ends in catch', code: 'wrap(fetch(url)).catch(handle);' },
      { name: 'a wrapped promise whose chain ends in finally', code: 'wrap(fetch(url)).finally(handle);' },
    ...
    invalid: [
      ...
      { name: 'a computed then access is the same protocol', code: 'p["then"]((r) => r.json());', errors: 1 },
      { name: 'a wrapper chained through a computed then', code: 'wrap(fetch(url))["then"](handle);', errors: 1 },
      {
        /**
      ...
      { name: 'an immediately-invoked async function', code: '(async function () { return 1; })();', errors: 1 },
      { name: 'an async arrow bound to a const', code: 'const load = async () => {};\nload();', errors: 1 },
      {
