...
All notable changes to `eslint-plugin-maintainability` are documented here.
...
- **🐛 Fix** — **🐛 Fix** — `no-unhandled-promise` silenced a promise passed as an argument
  ...
  `eslint-plugin-reliability`; the two rules share a shape but not a code path,
  and two of the branches are dead in one and live in the other — which is why
  the fix was made and verified separately in each.
...
- [#599](https://github.com/ofri-peretz/eslint/pull/599) [`43db150`](https://github.com/ofri-peretz/eslint/commit/43db150127c7729f8b4099655fd66d0ea6d4a480) Thanks [@ofri-peretz](https://github.com/ofri-peretz)! - `identical-functions` requires an **exact** match on short bodies.
...
- [#595](https://github.com/ofri-peretz/eslint/pull/595) [`05c2267`](https://github.com/ofri-peretz/eslint/commit/05c22675f07e69e1f682148e8064c7e699b1260f) Thanks [@ofri-peretz](https://github.com/ofri-peretz)! - `identical-functions` no longer erases the things that tell two functions
  ...
  **Comments ran last**, after whitespace had been collapsed onto one line — at
  which point `//.*` deletes from the first line comment to the _end of the
  function_. Two bodies were compared as their opening lines.
