...
All notable changes to `eslint-plugin-conventions` are documented here.
...
- **🐛 Fix** — consistent-existence-index-check no longer instructs a rewrite it refuses to perform.
...
- **🐛 Fix** — `filename-case` stops prescribing a rename it would reject
...
- **💥 Breaking** — `consistent-existence-index-check` defaults to `Object.hasOwn`, not `in`
...
- **🐛 Fix** — `consistent-existence-index-check` will not splice a sequence expression
...
- **🐛 Fix** — `prefer-dependency-version-strategy` no longer reads a manifest as a dependency map
...
- **💥 Breaking** — `recommended` no longer enables `no-magic-numbers` ([#694](https://github.com/ofri-peretz/eslint/pull/694))
...
- **✨ Feature** — Three rules no longer report inside generated files: `conventions/no-magic-numbers`, `conventions/no-commented-code` and `modernization/prefer-template-literal`. ([#691](https://github.com/ofri-peretz/eslint/pull/691))
  ...
  mean _edit this file_ — and the next codegen run overwrites the edit. The
