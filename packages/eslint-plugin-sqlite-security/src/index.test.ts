...
describe('eslint-plugin-sqlite-security plugin interface', () => {
  it('should export correct meta information', () => {
    expect(plugin.meta).toBeDefined();
    expect(plugin.meta?.name).toBe('eslint-plugin-sqlite-security');
  ...
  describe('configurations', () => {
    it('should provide recommended configuration', () => {
      ...
      expect(configs.recommended.plugins?.['sqlite-security']).toBeDefined();

      const recommendedRules = configs.recommended.rules || {};
      Object.keys(recommendedRules).forEach((ruleName) => {
        expect(ruleName).toMatch(/^sqlite-security\//);
      });

      expect(recommendedRules['sqlite-security/no-unsafe-query']).toBe('error');
    });

    it('should provide flagship configuration', () => {
      expect(configs.flagship).toBeDefined();
      expect(configs.flagship.rules?.['sqlite-security/no-unsafe-query']).toBe('error');
    });

    it('should provide strict configuration', () => {
      expect(configs.strict).toBeDefined();
      expect(configs.strict.plugins?.['sqlite-security']).toBeDefined();
      ...
      expect(strictRules['sqlite-security/no-unsafe-query']).toBe('error');
