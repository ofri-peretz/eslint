        ...
        if (depth < 10) {
          ...
          if (currentNode.type === 'BlockStatement' && currentNode.body) {
            ...
            if (
              ...
              currentNode.openingElement.attributes.forEach(
            ...
            currentNode.declarations.forEach(
    ...
    function analyzeClassComponent(node: TSESTree.ClassDeclaration) {
      ...
      for (const member of node.body.body) {
        if (
          ...
          linesOfCode = body.loc
            ? body.loc.end.line - body.loc.start.line + 1
            : 0;

          function analyzeRenderNode(
            currentNode: TSESTree.Node,
            depth = 0,
            visited = new Set<TSESTree.Node>(),
          ) {
      ...
      if (extendsComponent && !extendsPureComponent) {
        ...
        if (
          propsCount >= minPropsForPureComponent ||
          hasExpensiveComputations
        ) {
          context.report({
      ...
      if (suggestUseCallback && hasInlineFunctions && type !== 'class') {
        context.report({
          ...
          data: {
            componentName,
            reason:
              'has inline event handlers that create new functions on each render',
          },
        });
      }
