...
export const noInternalModules = createRule<RuleOptions, MessageIds>({
  ...
  meta: {
    ...
    messages: {
      internalModuleImport: formatLLMMessage({
        ...
        description:
          'Import "{{importPath}}" exceeds maximum allowed depth ({{depth}} > {{maxDepth}})',
        severity: 'MEDIUM',
    ...
    schema: [
      {
        ...
        properties: {
          maxDepth: {
            ...
            default: 1,
            description:
              'Maximum allowed depth of module imports (0 = only root, 1 = one level deep)',
          },
          allow: {
            type: 'array',
            items: { type: 'string' },
            default: [],
            description:
              'Glob patterns for paths that are allowed regardless of depth',
          },
          forbid: {
            type: 'array',
            items: { type: 'string' },
          ...
          strategy: {
            ...
            default: 'error',
            description:
              'How to handle violations: error (report), warn (report), autofix (fix automatically), suggest (provide suggestions)',
          },
        },
  ...
  create(context: TSESLint.RuleContext<MessageIds, RuleOptions>) {
    ...
    function checkImport(importPath: string, node: TSESTree.Node) {
      ...
      const isForbidden =
        forbid.length > 0 && matchesAnyPattern(importPath, forbid);
