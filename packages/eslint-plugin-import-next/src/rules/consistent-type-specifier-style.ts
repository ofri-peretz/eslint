      ...
      return {
        ImportDeclaration(node: TSESTree.ImportDeclaration) {
          ...
          if (style === 'prefer-inline') {
            ...
            // A default binding (`import yargs, { type Argv } from 'yargs'`) is not an
            // ImportSpecifier, so it is absent from `namedSpecifiers` — and the fixer
            // rebuilds the whole statement from `namedSpecifiers` alone. Rewriting would
            // therefore DELETE the default import and leave every use of it below as an
              ...
              context.report({
                ...
                fix: rebuildWouldDropABinding
                  ...
                  : function (fixer: TSESLint.RuleFixer) {
                      ...
                      const specifiersText = namedSpecifiers
                        .map((spec: TSESTree.ImportSpecifier) =>
                          getSpecifierText(spec),
                        )
                        .join(', ');
