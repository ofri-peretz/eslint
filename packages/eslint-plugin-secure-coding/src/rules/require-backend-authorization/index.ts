 ...
 * not match `roleplay` and `admin` must not match `adminEmail`.
 *
 ...
 * `accessLevel`, `tier` and `scope` would trade that false negative for false
 * positives on every pricing tier and OAuth scope in the ecosystem, so the miss
 * is documented rather than papered over.
 */
const AUTH_PROPERTIES = new Set([
  'role',
  'roles',
  'isAdmin',
  'isAuthenticated',
 ...
 * code runs. This is a runtime's closed global surface, not a variable's
 * spelling.
 */
  ...
  create(context) {
    ...
    return {
      ...
      MemberExpression(node: TSESTree.MemberExpression) {
        ...
        if (property === undefined || !AUTH_PROPERTIES.has(property)) return;
      ...
      'Program:exit'(program: TSESTree.Program) {
        ...
        const importsServerModule = program.body.some(
          (statement) =>
            ...
            SERVER_MODULES.has(String(statement.source.value)),
        );
        ...
        const touchesBrowserGlobal = sourceCode
          .getScope(program)
          .through.some((reference) => BROWSER_GLOBALS.has(reference.identifier.name));
