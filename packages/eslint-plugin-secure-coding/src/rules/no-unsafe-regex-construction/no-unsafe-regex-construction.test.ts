...
describe('no-unsafe-regex-construction', () => {
  describe('Valid Code', () => {
    ruleTester.run(
      'valid - safe regex construction',
      noUnsafeRegexConstruction,
      {
        valid: [
          'const regex = /^[a-z]+$/;',
  ...
  describe('Invalid Code - Unsafe Regex Construction', () => {
    ruleTester.run(
      'invalid - user-controlled regex patterns',
      noUnsafeRegexConstruction,
      {
        valid: [],
        invalid: [
                ...
                suggestions: [
                  {
                    ...
                    output:
                      'const regex = new RegExp((req.query.pattern).replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&"));',
                  },
                ],
              },
            ],
  ...
  describe('Pattern Length Limits', () => {
    ruleTester.run(
      'pattern length - maxPatternLength option',
      noUnsafeRegexConstruction,
      {
        valid: [
  ...
  describe('allowLiterals Option', () => {
    ruleTester.run(
      'allowLiterals - literal string handling',
      noUnsafeRegexConstruction,
      {
        valid: [
  ...
  describe('Dynamic Flags Detection', () => {
    ruleTester.run(
      'dynamic flags - second argument checks',
      noUnsafeRegexConstruction,
      {
        valid: [
  ...
  describe('Trusted Escaping Functions', () => {
    ruleTester.run(
      'escaping - custom trusted functions',
      noUnsafeRegexConstruction,
      {
        valid: [
          // Custom trusted escaping function
          {
            ...
            options: [
              { trustedEscapingFunctions: ['myCustomEscape', 'escapeRegex'] },
            ],
          },
        ],
        invalid: [
  ...
  describe('Edge Cases', () => {
    ...
    ruleTester.run(
      'rule partition: attributed taint reports, bare identifiers do not',
      noUnsafeRegexConstruction,
      {
        valid: [
    ...
    ruleTester.run(
      'trusted function wraps the entire new RegExp(...) call',
      noUnsafeRegexConstruction,
      {
        valid: [
          {
 ...
 */
ruleTester.run(
  'lock: a RegExp clone is not a new pattern',
  noUnsafeRegexConstruction,
  {
    valid: [
      {
 ...
 */
ruleTester.run(
  'no-unsafe-regex-construction-ts-cast-taint',
  noUnsafeRegexConstruction,
  {
    valid: [`const re = new RegExp('^[a-z]+$' as string, 'i');`],
    invalid: [
      {
