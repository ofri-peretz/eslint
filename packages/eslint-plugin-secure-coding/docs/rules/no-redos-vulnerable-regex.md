...
| `reportSecondDegreePolynomial` | `boolean` | `true` | Report quadratic backtracking. On by default — the CWE-1333 corpus pins a degree-2 fixture as must-detect. Turn it off only for patterns that provably run over short, sized input; doing so costs recall. Requires the recheck oracle. |
