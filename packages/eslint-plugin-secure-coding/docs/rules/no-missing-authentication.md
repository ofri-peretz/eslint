...
| `ignorePatterns` | `string[]` | `["login","logout","signin","sign-in","signup","sign-up","register","forgot-password","reset-password","password-reset","verify-email","health","healthz","readyz","livez","alive","liveness","readiness","heartbeat","status","metrics","ping","favicon","webhook","/public/","oauth","callback"]` | Route paths that are public by definition and need no authentication. Replaces the built-in list; supplying it also widens matching from the route path to the whole registration text. |
| `routerNameWords` | `string[]` | `["app","router","route","server","express","fastify","koa","hapi"]` | Object names that denote an HTTP application or router, compared as WHOLE WORDS and never as a substring. Read only when the binding cannot be resolved to a framework factory call. Replaces the built-in list. |
| `additionalRouterNameWords` | `string[]` | `[]` | Extra router-object name words, on top of `routerNameWords`. |
