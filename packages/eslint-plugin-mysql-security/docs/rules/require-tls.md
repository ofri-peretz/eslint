...
> **Keywords:** cleartext transmission, TLS, SSL, CWE-319, OWASP A02:2021, mysql2 / mysql, mysql, rejectUnauthorized, certificate validation, man in the middle
...
mysql2 accepts the verification flag at the top level as well as inside `ssl`, and its URL form takes `?ssl=false`. All three shapes are checked. `ssl: "Amazon RDS"` selects a bundled CA and is correct, not a finding.
...
// ❌ hoisted to the top level — mysql2 accepts it there too
const c3 = mysql.createConnection({ database: 'app', rejectUnauthorized: false });
```
