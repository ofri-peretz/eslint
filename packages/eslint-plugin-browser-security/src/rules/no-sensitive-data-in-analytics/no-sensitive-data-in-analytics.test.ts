...
ruleTester.run('no-sensitive-data-in-analytics', noSensitiveDataInAnalytics, {
  ...
  invalid: [
    // Email in analytics
    {
      name: 'an email address sent to the analytics provider',
      code: "analytics.track('signup', { email: user.email })",
      errors: [{ messageId: 'violationDetected' }],
    },
    {
      code: "analytics.track('login', { userEmail: email })",
      errors: [{ messageId: 'violationDetected' }],
    },
    // SSN
    {
      code: "analytics.track('verify', { ssn: userSSN })",
      errors: [{ messageId: 'violationDetected' }],
    },
    // Credit card
    {
      code: "analytics.track('purchase', { creditcard: card })",
      errors: [{ messageId: 'violationDetected' }],
    },
    // Password
    {
      code: "analytics.track('auth', { password: pwd })",
      errors: [{ messageId: 'violationDetected' }],
    },
    // Phone
    {
      code: "analytics.track('contact', { phone: number })",
      errors: [{ messageId: 'violationDetected' }],
    },
    // Address
    {
      code: "analytics.track('order', { address: addr })",
      errors: [{ messageId: 'violationDetected' }],
    },
    // Multiple sensitive fields
    {
      code: "analytics.track('profile', { email: e, phone: p })",
      errors: [
...
ruleTester.run(
  'no-sensitive-data-in-analytics — adversarial',
...
ruleTester.run(
  'no-sensitive-data-in-analytics — refusals',
  noSensitiveDataInAnalytics,
  {
    valid: [
 ...
 */
ruleTester.run(
  'no-sensitive-data-in-analytics — sensitiveFields',
  noSensitiveDataInAnalytics,
  {
    valid: [
