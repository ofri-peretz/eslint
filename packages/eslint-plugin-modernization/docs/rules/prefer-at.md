...
> **Not auto-fixed.** Three of the reported shapes are left for you to edit,
...
> - `array[array.length - n]` as the callee of a call or `new`, or the tag of a
>   tagged template. `obj.fns.at(-1)()` calls with `this` undefined where
>   `obj.fns[obj.fns.length - 1]()` passed `obj.fns`, and `new ctors.at(-1)()`
>   parses as `new (ctors.at)(-1)` and throws.
>
> All still report — `array[-1]` is almost always a mistake — but naming the
