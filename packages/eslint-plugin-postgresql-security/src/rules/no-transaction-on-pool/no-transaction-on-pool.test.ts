...
describe('no-transaction-on-pool', () => {
  ...
  describe('corpus regressions', () => {
    ruleTester.run(
      'the receiver is resolved, not spelled',
      noTransactionOnPool,
      {
        valid: pg([
    ...
    ruleTester.run(
      'the statement is parsed, not upper-cased and split',
      noTransactionOnPool,
      {
        valid: pg([
    ...
    ruleTester.run(
      'scoped PostgreSQL packages count too',
      noTransactionOnPool,
      {
        valid: [],
        invalid: [
          ...
          // `pool.query({ text })` written with a computed key. The config-object
          // arm hand-rolled Identifier-or-Literal and caught two of the four
