...
describe('eslint-plugin-typeorm-security plugin interface', () => {
  it('should export correct meta information', () => {
    expect(plugin.meta).toBeDefined();
    expect(plugin.meta?.name).toBe('eslint-plugin-typeorm-security');
  ...
  describe('configurations', () => {
    it('should provide recommended configuration', () => {
      ...
      expect(configs.recommended.plugins?.['typeorm-security']).toBeDefined();

      const recommendedRules = configs.recommended.rules || {};
      Object.keys(recommendedRules).forEach((ruleName) => {
        expect(ruleName).toMatch(/^typeorm-security\//);
      });

      expect(recommendedRules['typeorm-security/no-unsafe-query']).toBe('error');
    });

    it('should provide flagship configuration', () => {
      expect(configs.flagship).toBeDefined();
      expect(configs.flagship.rules?.['typeorm-security/no-unsafe-query']).toBe('error');
    });

    it('should provide strict configuration', () => {
      expect(configs.strict).toBeDefined();
      expect(configs.strict.plugins?.['typeorm-security']).toBeDefined();
      ...
      expect(strictRules['typeorm-security/no-unsafe-query']).toBe('error');
