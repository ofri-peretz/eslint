...
description: Disallow literal database passwords in TypeORM connection configuration, including credentials embedded in a connection URL.
...
> **Keywords:** hardcoded credentials, CWE-798, OWASP A07:2021, TypeORM, typeorm, connection string, DATABASE_URL, secret management, git history
...
Detects a database password written as a literal — as a config property, or embedded in a connection URL. This rule is part of [`eslint-plugin-typeorm-security`](https://www.npmjs.com/package/eslint-plugin-typeorm-security).
...
new DataSource({ type: 'postgres', url: 'postgres://app:s3cret@db.internal/app' });
```
...
new DataSource({ type: 'postgres', host, username, password: process.env.DB_PASSWORD });
```
