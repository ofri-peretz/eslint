...
ruleTester.run(
  'detect-non-literal-fs-filename — computed keys',
  detectNonLiteralFsFilename,
  {
    valid: [
      ...
      /*
       * The SUPPRESSION direction, which is where a blind gate costs precision
       * rather than recall.
       *
       * `path.basename(...)` is the remediation this rule's own message
       * recommends, so a call to it must not itself be reported. Six gates read
       * `property.type === Identifier` alongside a `propertyName(...)` call —
       * strictly narrower, and evaluated first — so the resolver never ran and
       * `path['basename'](userPath)` was reported as unsanitized. The rule was
       * flagging code that had taken its own advice.
       *
       * Measured both ways: with the guard restored this reports 1 finding, and
       * `path[pick](...)` reports in either state, which is correct — a key
       * chosen at runtime cannot be shown to be `basename`.
       *
       * Only the bare `path.basename(...)` wrapper is pinned here. The nested
       * `path.resolve(SAFE_DIR, path.basename(x))` form — the rule's own
       * documented `safePattern` — reports in BOTH spellings, so whatever that
       * is, it is not a computed-key blind spot and does not belong to this
       * change.
       */
      {
        name: 'path["basename"] sanitizes exactly as path.basename does',
        code: `import fs from "fs";\nimport path from "path";\nfs.readFileSync(path["basename"](req.query.f));`,
      },
    ],
    invalid: [
      {
