...
describe('no-shell-injection', () => {
  ...
  describe('Constant Folding', () => {
    ruleTester.run(
      'folded interpolations are not injection',
      noShellInjection,
      {
        valid: [
        ...
        invalid: [
          ...
          {
            // A private field read, which needs an enclosing class — `cfg.#p` at
            // top level is a private name outside a class body and cannot parse.
            // typescript-eslint tolerated it, so this case ran on code that
            // could never exist; the class is what makes it the interpolation
            // through a private member it was written to be.
            name: 'a private field interpolated into the command string',
            code: "import { exec, execSync, spawn, spawnSync, execFile, execFileSync } from 'node:child_process';\nclass C { #p = '/srv'; run() { execSync(`ls ${this.#p}`); } }",
            errors: [{ messageId: 'shellInjection' }],
          },
  ...
  describe('Benchmark FP/FN Regression', () => {
    ruleTester.run(
      'benchmark regression - no FPs on safe patterns',
      noShellInjection,
      {
        valid: [
 ...
 */
ruleTester.run(
  'no-shell-injection-requires-module-evidence',
  noShellInjection,
  {
    valid: [
