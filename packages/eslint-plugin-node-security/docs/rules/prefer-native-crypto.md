...
This rule takes no options. Severity is set the ESLint way, by the config entry:
...
> **Removed:** a `severity: 'error' | 'warn'` option was previously documented
> here. `create()` never read it and no rule can — ESLint decides severity from
> the config entry, not from rule options — so setting it changed nothing while
> the schema accepted it silently. It now fails validation instead.

## Password hashing is a separate case

`bcryptjs` and `bcrypt-nodejs` report `preferNativePasswordHash`, not
`preferNative`, because **`node:crypto` has no bcrypt**. Migrating a password
hash to `crypto.createHash` replaces a deliberately-slow KDF with a fast digest
— [CWE-916](https://cwe.mitre.org/data/definitions/916.html), a worse bug than
the one being reported. The remedy is the native `bcrypt` binding, `argon2`, or
`crypto.scrypt`.
