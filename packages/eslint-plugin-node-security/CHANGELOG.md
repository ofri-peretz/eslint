...
- **🐛 Fix** — path guards no longer leak across functions; destructured `basename`/`sep` recognised
...
- **🐛 Fix** — `no-weak-hash-algorithm` sees `ripemd160`, not just `ripemd`
...
- **🐛 Fix** — A computed or quoted property key is still the same property.
