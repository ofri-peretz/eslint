 ...
 * `require('eslint-plugin-pg/oxlint')` view, it sits at the top of
 * `module.exports` — the `{ meta, rules }` shape oxlint's JS plugin loader
 * expects.
 */
describe('eslint-plugin-pg/oxlint sub-export', () => {
  ...
  it('re-exports the plugin object (meta + rules at top level)', async () => {
    ...
    expect(oxlintPlugin.meta?.name).toBe('eslint-plugin-pg');
