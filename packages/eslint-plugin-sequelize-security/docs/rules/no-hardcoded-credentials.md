...
description: Disallow literal database passwords in Sequelize connection configuration, including credentials embedded in a connection URL.
...
> **Keywords:** hardcoded credentials, CWE-798, OWASP A07:2021, Sequelize, sequelize, connection string, DATABASE_URL, secret management, git history
...
Detects a database password written as a literal — as a config property, or embedded in a connection URL. This rule is part of [`eslint-plugin-sequelize-security`](https://www.npmjs.com/package/eslint-plugin-sequelize-security).
...
```ts
// ❌ literal password
new Sequelize({ dialect: 'postgres', host, username, password: 'hunter2' });

// ❌ the same secret, hidden in a URL
new Sequelize('postgres://app:s3cret@db.internal/app');
```
...
new Sequelize({ dialect: 'postgres', host, username, password: process.env.DB_PASSWORD });
```
...
```ts
// eslint-disable-next-line sequelize-security/no-hardcoded-credentials -- ephemeral test container
new Sequelize({ dialect: 'postgres', host, username, password: 'hunter2' });
```
