/**
 * Tests for sequelize-security/no-hardcoded-credentials
    ...
    valid: [
      {
        ...
        code: DRIVER + `new Sequelize({ dialect: 'postgres', host, database, password: config.dbPassword })`,
      },
      {
        name: 'the fix — the password comes from the environment',
        code: DRIVER + `new Sequelize({ dialect: 'postgres', host, database, password: process.env.DB_PASSWORD })`,
      },
      {
        // Same call shape, no sequelize import: this is somebody else's API.
        name: 'silent without the driver import',
        code: `new Sequelize({ dialect: 'postgres', host, database, password: 'hunter2' })`,
      },
    ],
    invalid: [
      {
        name: 'a template literal with no interpolation is still a literal',
        code: DRIVER + `new Sequelize({ dialect: 'postgres', host, database, password: \`hunter2\` })`,
      ...
      },
      {
        ...
        code: DRIVER + `new Sequelize('postgres://admin:hunter2@db.internal:5432/app')`,
