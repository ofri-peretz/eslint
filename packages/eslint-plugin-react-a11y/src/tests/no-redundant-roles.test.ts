...
describe('no-redundant-roles', () => {
  describe('Valid Code', () => {
    ruleTester.run('valid - no redundant roles', noRedundantRoles, {
      valid: [
        {
          // A role decided at runtime cannot be compared to the implicit one.
          // Reporting would mean guessing what `roleName` holds, and the guess
          // would be wrong exactly when it mattered.
          name: 'a role supplied by an expression, which the AST cannot read',
          code: '<nav role={roleName} />',
        },
        {
          name: 'a role from a template with an interpolation',
          code: '<nav role={`${prefix}-navigation`} />',
        },
        {
          /**
  ...
  describe('Invalid Code', () => {
    ruleTester.run('invalid - redundant roles', noRedundantRoles, {
      ...
      invalid: [
        ...
        {
          name: "role='main' on a main element",
