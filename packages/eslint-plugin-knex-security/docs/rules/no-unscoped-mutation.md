...
description: Require a chained `.where()` on Knex delete and update builders, so a bulk mutation cannot rewrite or delete every row in the table.
tags: ['security', 'knex']
category: security
severity: high
cwe: CWE-284
...
> **Keywords:** unscoped mutation, mass deletion, CWE-284, OWASP A01:2021, Knex, knex, bulk delete, bulk update
...
Detects Knex bulk mutations that reach every row in the table. This rule is part of [`eslint-plugin-knex-security`](https://www.npmjs.com/package/eslint-plugin-knex-security).
...
shows up once it has run against production data. Knex's `update()` argument is the values object rather than a filter, and `del()` takes none — the scope always arrives as a chained `.where*()` clause. Every spelling of the `where` family counts.
...
- [Knex documentation](https://knexjs.org/guide/query-builder.html#del-delete)
