 ...
 * `require('eslint-plugin-knex-security/oxlint')` view, it sits at the top of
 * `module.exports` — the `{ meta, rules }` shape oxlint's JS plugin loader
 * expects.
 */
describe('eslint-plugin-knex-security/oxlint sub-export', () => {
  ...
  it('re-exports the plugin object (meta + rules at top level)', async () => {
    ...
    expect(oxlintPlugin.meta?.name).toBe('eslint-plugin-knex-security');
