...
description: Disallow writing an inbound request object straight to the database through Prisma, which lets the caller set every column the model exposes.
...
> **Keywords:** mass assignment, CWE-915, OWASP A04:2021, Prisma, prisma, req.body, privilege escalation, over-posting, allowlist, isAdmin
...
Detects an inbound request object — or a spread of one — reaching a Prisma write. This rule is part of [`eslint-plugin-prisma-security`](https://www.npmjs.com/package/eslint-plugin-prisma-security).
...
Prisma carries the row under `data`, and `upsert` carries two payloads (`create` and `update`) — all three are checked.
...
await prisma.user.update({ where: { id }, data: { name: req.body.name } });
...
- **A file that never imports prisma.** The driver import is the gate that keeps
...
```ts
// eslint-disable-next-line prisma-security/no-mass-assignment -- payload is built in-process, not from a request
await prisma.user.update({ where: { id }, data: req.body });
```
