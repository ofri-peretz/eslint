...
function loadPresetRules() {
  for (const dir of fs
    ...
    Object.assign(
      ...
      cfg.rules ??
        (Array.isArray(cfg)
          ? Object.assign({}, ...cfg.map((c: any) => c.rules ?? {}))
          : {}),
    );
  }
}
...
async function main() {
  ...
  console.error(
    `[${Object.keys(plugins).length} plugins, ${enabled.size} rules on preset "${PRESET}"]`,
  );

  const walk = (d: string, out: string[] = []): string[] => {
    for (const e of fs.readdirSync(d, { withFileTypes: true })) {
      if (
        /node_modules|\.git|dist|build|coverage|fixtures|__snapshots__/.test(
          e.name,
        )
      )
        continue;
      ...
      if (e.isDirectory()) walk(p, out);
        ...
        /\.(js|mjs|cjs|jsx|ts|mts|cts|tsx)$/.test(e.name) &&
        !/\.min\./.test(e.name)
      )
        out.push(p);
    }
  ...
  for (const target of targets) {
    ...
    for (const h of hits)
      byRule.set(h.rule, [...(byRule.get(h.rule) ?? []), h]);

    const suppressed = [...byRule].filter(([r]) => knownFp.has(r));
    const noisy = [...byRule].filter(
      ([r, hs]) => !knownFp.has(r) && hs.length > NOISE_FLOOR,
    );
    const triage = [...byRule]
    ...
    if (suppressed.length)
      console.log(
        `\n  suppressed (already proven FP, in fp-gate baseline):\n` +
          suppressed
            .map(([r, hs]) => `    ${String(hs.length).padStart(4)}  ${r}`)
            .join('\n'),
      );
