 ...
 * (The design-system philosophies used to also project into the ESLint
 * docs app; that surface was removed — they belong to the design
 * system, not the ESLint docs.)
 *
...
// 8 named clusters that organise the philosophies.
...
// rendering (Storybook nested title, index sections) derives from this
// single map.
interface Category {
  label: string;
...
function loadPhilosophy(sourcePath: string): Philosophy {
  ...
  return { sourcePath, slug, title, description, body, hash };
}
