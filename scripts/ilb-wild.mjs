 ...
 * fired), and per-repo lint cost. See benchmarks/ILB_NAMING.md.
 *
 * Methodology:
...
function buildSummary(date, normalizedResults) {
  ...
  for (const r of ok) {
    for (const [p, c] of Object.entries(r.pluginCoverage || {})) {
      if (!pluginAgg[p]) pluginAgg[p] = { totalRules: c.total, repos: 0, firedAcrossRepos: new Set() };
      pluginAgg[p].repos++;
...
function renderSummaryMd(s) {
  ...
  const pluginRows = Object.entries(s.pluginRollup)
