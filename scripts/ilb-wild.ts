...
import { cloneRepo, resolveBenchDir } from '../benchmarks/lib/clone-repo';
...
const BENCH_DIR = resolveBenchDir(ROOT);
...
async function main() {
  ...
  for (const repo of repos) {
    ...
    try {
      const dir = cloneRepo(repo, BENCH_DIR);
...
// ── Repo lifecycle ──────────────────────────────────────────────────
// `cloneRepo` and `safeGit` live in benchmarks/lib/clone-repo.ts so the
// ilb-perf-import-nestjs suite can reuse the same shallow-clone-with-
// fallback logic.
