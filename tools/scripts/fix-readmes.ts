...
const DESCRIPTIONS: Record<string, string> = {
  ...
  'eslint-plugin-node-security':
    'Security rules for Node.js core modules (fs, child_process, crypto, etc).',
...
function processPackage(pkg: string): void {
  ...
  lines.forEach((line: string) => {
    ...
    if (match && !line.includes('---')) {
      ...
      const typeStatus: TypeStatus | null = cells.some((c: string) =>
        c.includes('🟢'),
      )
        ? 'unaware'
  ...
  if (fs.existsSync(rulesDir)) {
    realRules = fs
      ...
      .filter((d: fs.Dirent) => {
        ...
        if (d.isFile() && (d.name.endsWith('.ts') || d.name.endsWith('.js')))
          return true;
        return false;
      })
  ...
  output.push('<p align="center">');
  output.push(
    `  <a href="https://www.npmjs.com/package/${pkg}" target="_blank"><img src="https://img.shields.io/npm/v/${pkg}.svg" alt="NPM Version" /></a>`,
  );
  output.push(
    `  <a href="https://www.npmjs.com/package/${pkg}" target="_blank"><img src="https://img.shields.io/npm/dm/${pkg}.svg" alt="NPM Downloads" /></a>`,
  );
  output.push(
    `  <a href="https://opensource.org/licenses/MIT" target="_blank"><img src="https://img.shields.io/badge/License-MIT-yellow.svg" alt="Package License" /></a>`,
  );
  output.push(
    `  <a href="https://app.codecov.io/gh/ofri-peretz/eslint/components?components%5B0%5D=${pluginName}" target="_blank"><img src="https://codecov.io/gh/ofri-peretz/eslint/graph/badge.svg?component=${pluginName}" alt="Codecov" /></a>`,
  );
  output.push(
  ...
  output.push(
    `- Read the [guide](${docUrl}) at [eslint.interlace.tools](${baseUrl}). 📚`,
  );
  output.push('');
  output.push('```bash');
  ...
  customSections.forEach((section) => {
    ...
    if (
      ...
      section.header.includes('Documentation')
    )
      return;
  ...
  if (realRules.length > 0) {
    ...
    const generateRow = (ruleName: string) => {
      ...
      const desc =
        getMdxDescription(docsSubPath, ruleName) ||
        `Enforce ${ruleName.replace(/-/g, ' ')}`;
      ...
      const has = (key: keyof RuleMeta['flags']) =>
        meta?.flags && meta.flags[key]
          ? {
              recommended: '💼',
              warn: '⚠️',
              fixable: '🔧',
    ...
    if (hasCategories) {
      ...
      Object.entries(hasCategories).forEach(([catName, supportedRules]) => {
        ...
        const rulesInCat = supportedRules.filter((r: string) =>
          realRules.includes(r),
        );
...
`);
  output.push(`<p align="center">`);
  output.push(
    `  <a href="https://eslint.interlace.tools/docs/${docsSubPath}"><img src="https://eslint.interlace.tools/images/og-${pluginName}.png" alt="ESLint Interlace Plugin" width="100%" /></a>`,
  );
  output.push(`</p>`);
  output.push('');
