...
//
// The why/how/what beats replaced `## Philosophy`. The Philosophy block
// was identical in all thirty READMEs and told a reader nothing they could act on
// or disagree with, while sitting in the one slot that has to sell. The three
// beats are generated by `scripts/sync-readme-rules.ts` between the DOCTRINE
// markers, so they cannot drift apart across packages.
...
const DOCTRINE_FINGERPRINTS = [
  'A linter nobody reads protects nothing.',
  '**Evidence, not names.**',
...
function checkPlugin(pkg: string): Violation | null {
  ...
  if (!content.includes('/logos/interlace.svg" alt="Interlace" height="70"')) {
