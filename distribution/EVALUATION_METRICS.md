...
| **API-surface coverage** | For domain plugins: % of the *security-relevant* SDK / runtime API surface that is lint-aware. E.g. % of `node:crypto` exports our `node-security` plugin has rules for. Denominator excludes APIs that are not sinks under the plugin's threat model, each recorded in `outOfScope` with a written reason; the audit rejects reasons that argue from rarity, so a gap can't be relabelled into a higher score. | percent | ≥ 60% per vertical | Yes (per neighbor) | `npm run audit:api-surface` → `benchmark-results/api-surface-coverage.md`; per-plugin entries in `.agent/api-surface-manifest.json`; `:strict` variant fails CI when any plugin drops below floor |
