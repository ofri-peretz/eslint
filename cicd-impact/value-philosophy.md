...
| **6** | **140 of 217 Interlace rules (64.5%) use `context.getFilename()` / `context.getSourceCode()` / `context.getCwd()` — APIs removed in ESLint 10.** This is the same class of break we publicly cite as disqualifying for `eslint-plugin-security@4.0.0`. The CLAIMS.md row "Supports ESLint 8, 9, and 10" has been caveated to disclose the partial-support reality. Surfaced by [`cicd-impact/scripts/eslint10-compat-test.mjs`](../cicd-impact/scripts/eslint10-compat-test.mjs) and confirmed via static analysis. | **Migration runbook staged at [`cicd-impact/eslint10-migration-runbook.md`](eslint10-migration-runbook.md)** — pre-flight scope check + two-phase sed (replace bare old APIs, then collapse redundant fallback patterns) + 5-step verification (zero remaining old-API usage, type-check, unit tests, eslint10-compat-test, claims update). ESLint provides both APIs in v8.40+ and v9 for backwards compatibility, so the migration is safe to land before dropping v8 support. **Status: ⏸️ awaiting maintainer authorisation to run** — the runbook is reversible via `git checkout` / `git revert`. Tracked in `benchmarks/FP_FN_REMEDIATION_TRACKER.md` for FN regression coverage. | **High** — the credibility of the cross-version compatibility claim is a load-bearing assertion in CLAIMS.md and, by extension, in any docs / dev.to posts that cite "supports ESLint 10". Until fixed, the claim is structurally false in practice. The runbook reduces "fix" cost from "discovery + planning + execution" to just "execution" — closer to a one-command operation. |
