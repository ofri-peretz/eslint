...
BODY  (markdown, may contain shields.io badges in tables)
  4.  ## Description
  5.  why / how / what                    (generated, between DOCTRINE markers,
                                           three bullets — NO heading of its own)
...
**Theme variants — Interlace, ESLint and oxlint only.** Those three appear in every
row, so each ships a `-light` / `-dark` pair and is written as a `<picture>`:

```markdown
<picture><source media="(prefers-color-scheme: dark)" srcset="https://eslint.interlace.tools/logos/interlace-dark.svg"><img src="https://eslint.interlace.tools/logos/interlace-light.svg" alt="Interlace" height="90" /></picture>
```

**Ours are named by SURFACE, not by ink:** `-light` = for light surfaces (dark ink,
npm + GitHub light), `-dark` = for dark surfaces (light ink, GitHub dark). That is the
opposite of oxc's convention, where `oxc-light.svg` means light-*coloured* ink — a
collision that has already produced one wrong-variant commit here.

The `<img>` inside the `<picture>` is the `-light` file on purpose: it is the fallback
for any renderer that drops `<source>`, and npm's README background is always white.
The base `<mark>.svg` stays on disk for single-file consumers but must not appear in a
README — the gate rejects it.

Generated by `tools/scripts/make-theme-variants.mjs` (`npm run logos:variants`);
`npm run logos:variants:check` fails if a variant is stale or missing. The ~20 vendor
ecosystem marks keep a plain `<img>` — they are full-colour and read on both surfaces.
...
**Generated — never hand-edit.** `scripts/sync-readme-rules.ts` writes this between
`<!-- AUTO-GENERATED:DOCTRINE:START -->` and `:END`; run `npm run sync-readmes` to
refresh it. The gate rejects a hand-edited block.

Three bullets and one line of links, under **no heading of its own**:
...
That trade costs recall, and we measure it: [methodology](…) · [results](…) · […](…).
```

One sentence per bullet. If a bullet needs a second, it belongs in
DOCS_PHILOSOPHY.md or the benchmark docs, and the bullet links there.

This slot is the whole sell, and it is deliberately cheap. The reference is the
NestJS npm README — tagline, short Description, then the install command. A reader
arriving from npm wants `npm i`; everything above it is rent. Three shapes were
tried here and only this one is cheap enough:

| Shape | Why it failed |
| :--- | :--- |
| `## Philosophy` — "Interlace fosters **strength through integration** … a resilient fabric of code" | Identical in all thirty READMEs, and nothing a reader could act on or disagree with. Held the sell slot without selling. |
| three `##` sections, ~45 lines | Said something, but pushed Getting Started and the rule table below the fold on every package. |
| three `##` sections, ~16 lines | Three table-of-contents entries for one thought. |

The Philosophy text must not come back; the gate fails on its fingerprint. A plugin
may still carry a hand-written `## Why <ecosystem>-specific?` — that is the
per-plugin why, and it is a real section.
...
```markdown
<p align="center">
  <a href="https://eslint.interlace.tools" target="blank"><picture><source media="(prefers-color-scheme: dark)" srcset="https://eslint.interlace.tools/logos/interlace-dark.svg"><img src="https://eslint.interlace.tools/logos/interlace-light.svg" alt="Interlace" height="70" /></picture></a>
</p>
```

Interlace mark only — the ESLint mark pairing lives in the header (1), not
here. **The same `/logos/interlace-{light,dark}.svg` pair as the header**, smaller (~70px).
