...
Single-shot timings on `~/repos/ofriperetz.dev/oos/`. Full data for the 2026-05-10 run is in [`benchmark-results/history.ndjson`](../benchmark-results/history.ndjson) — the dated snapshot under `benchmarks/results/ilb-flagship/` is NOT a durable citation, because the weekly benchmark keeps only the three most recent per suite and prunes the rest. What changed vs v2 (2026-05-09):
